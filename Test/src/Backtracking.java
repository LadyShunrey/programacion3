...
public class Backtracking<T> {
	...
	boolean esSolucion(GrafoNoDirigido<T> grafo, List<T> visitadas) {
		return grafo.cantidadVertices() == visitadas.size();
	}

	List<Arco<T>> obtenerPosiblesArcos(GrafoNoDirigido<T> grafo, List<T> visitadas, ArrayList<Arco<T>> arcosUsados) {
	...
	public boolean backtracking(GrafoNoDirigido<T> grafo, T estacionActual, int metrosDeRedActualEnConstruccion,
